...
若需獨立驗證 UI，將 `assets/.env` 中的 `API_BASE_URL` 留空即可使用本機示範 response；真實 API contract 請參考根目錄 `AGENTS.md` 的 `/api/chat`。

## 行為

- 啟動時取得位置；每次請求都以固定 `client_id`（預設為 `1`）傳給後端。
- 使用者以自然語言傳訊；前端顯示 `collecting_info` 與 `error` 的 `reply_text`，並用 `route_ready` 的結構化資料組成路線摘要。
- `route_ready` 顯示一條推薦路線、後端 metrics／warnings、動態事件摘要，並在地圖標示沿線警察局與可求助據點的名稱。
