...
或各分店彼此距離很近、走路路線差異不大。只要能合理判斷，就直接選用
該分店繼續進行（呼叫工具時把地點說清楚，例如「大遠百（台北店）」），
不要為了消歧而多問一輪。只有在真的無法判斷時（例如分店分散在明顯不同
的城市或區域，且沒有任何線索可以縮小範圍）才詢問使用者，並且只列出你
判斷最可能的少數幾個選項（2 到 4 個即可），不要把所有分店都列出來要
使用者自己選。
""".strip()
